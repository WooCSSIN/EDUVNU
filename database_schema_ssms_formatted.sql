...
-- 1. Tạo bảng auth_group
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[auth_group]') AND type in (N'U'))
BEGIN
...
-- 2. Tạo bảng django_content_type
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[django_content_type]') AND type in (N'U'))
BEGIN
...
-- 3. Tạo bảng auth_permission
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[auth_permission]') AND type in (N'U'))
BEGIN
...
(
    ...
    [end_date] date NULL,
    [visibility_status] nvarchar(20) NOT NULL,
...
(
    ...
    [title] nvarchar(255) NOT NULL,
    [order] int NOT NULL CONSTRAINT courses_chapter_order_b9ff7429_check CHECK ([order] >= 0),
    [created_at] datetimeoffset NOT NULL
);
GO
...
(
    ...
    [order] int NOT NULL CONSTRAINT courses_question_order_087574ce_check CHECK ([order] >= 0)
);
GO
...
(
    ...
    [passed] bit NOT NULL,
    [draft_answers] nvarchar(max) NULL CONSTRAINT courses_quizattempt_draft_answers_dbbb4f66_check CHECK ((ISJSON ("draft_answers") = 1)),
...
(
    ...
    [title] nvarchar(255) NOT NULL,
    [order_number] int NOT NULL CONSTRAINT courses_lesson_order_number_c57fa89d_check CHECK ([order_number] >= 0),
    [video_url] nvarchar(255) NULL,
    [document_file] nvarchar(100) NULL,
    [content] nvarchar(max) NOT NULL,
    [is_active] bit NOT NULL
);
GO

CREATE TABLE [courses_lessoncomment]
(
    [id] bigint NOT NULL PRIMARY KEY IDENTITY (1, 1),
...
(
    ...
    [lesson_id] bigint NOT NULL,
    [status] nvarchar(20) NOT NULL,
    [time_spent] int NOT NULL CONSTRAINT courses_userprogress_time_spent_c0d173b9_check CHECK ([time_spent] >= 0),
    [last_accessed] datetimeoffset NOT NULL
);
GO
...
CREATE TABLE [courses_contactmessage]
(
    [id] bigint NOT NULL PRIMARY KEY IDENTITY (1, 1),
    [name] nvarchar(100) NOT NULL,
    [email] nvarchar(254) NOT NULL,
...
CREATE TABLE [courses_instructorwallet]
(
    [id] bigint NOT NULL PRIMARY KEY IDENTITY (1, 1),
...
CREATE TABLE [courses_wallettransaction]
(
    [id] bigint NOT NULL PRIMARY KEY IDENTITY (1, 1),
...
CREATE TABLE [courses_withdrawalrequest]
(
    [id] bigint NOT NULL PRIMARY KEY IDENTITY (1, 1),
    [user_id] bigint NOT NULL,
...
CREATE TABLE [courses_courseannouncement]
(
    [id] bigint NOT NULL PRIMARY KEY IDENTITY (1, 1),
    [course_id] bigint NOT NULL,
    [title] nvarchar(255) NOT NULL,
...
(
    ...
    [order] int NOT NULL CONSTRAINT courses_faq_order_6b9b599d_check CHECK ([order] >= 0),
    [is_active] bit NOT NULL,
    [created_at] datetimeoffset NOT NULL
);
GO
...
(
    ...
    [tuition] nvarchar(100) NOT NULL,
    [logo] nvarchar(50) NOT NULL,
    [instructor] nvarchar(200) NULL,
    [instructor_title] nvarchar(200) NULL,
    [skills] nvarchar(max) NOT NULL CONSTRAINT courses_degreeprogram_skills_c0713140_check CHECK ((ISJSON ("skills") = 1)),
    [curriculum] nvarchar(max) NOT NULL CONSTRAINT courses_degreeprogram_curriculum_55baa152_check CHECK ((ISJSON ("curriculum") = 1)),
    [videos] nvarchar(max) NOT NULL CONSTRAINT courses_degreeprogram_videos_2cd269ce_check CHECK ((ISJSON ("videos") = 1)),
    [created_at] datetimeoffset NOT NULL,
    [is_active] bit NOT NULL
);
GO
